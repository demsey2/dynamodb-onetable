...
export class Table {
  ...
  public updatedField: any;
  public genericModel: any;
  ...
  addModel(name: string, fields): void {
   ...
   * @returns
   */
  setContext(context = {}, merge = false): this {
   ...
   * @returns
   */
  async create(modelName: string, properties, params) {
   ...
   * @returns
   */
  async batchGet(batch, params: TableParams) {
    let result;
  ...
  log(type: string, message: string, context?: Record<string, unknown>, params?: Record<string, unknown>) {
    if (this.logger) {
  ...
  /*
    ...
    */
  defaultLogger(type: string, message: string, context: Record<string, unknown>): void {
